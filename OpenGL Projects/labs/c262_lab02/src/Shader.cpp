...
Shader::Shader(const char* vertexFilename, const char* fragmentFilename){
    ...
    /*YOUR ANSWER*/
	char* cstr_vertex_shader_source = load_text_file(vertexFilename);

    // Read in the fragment program file
    /*YOUR ANSWER*/
	char* cstr_fragment_shader_source = load_text_file(fragmentFilename);

    // Check to make sure neither one is null.  If so, print an error and exit
    /*YOUR ANSWER*/
	if (cstr_vertex_shader_source == NULL) {
		printf("ERROR *** failed to load %s, exiting\n", vertexFilename);
		exit(1);
	}

	if (cstr_fragment_shader_source == NULL) {
		printf("Error *** failed to load %s, exiting\n", fragmentFilename);
		exit(1);
	}
    ...
    /*YOUR ANSWER*/
	program_ID = link_shader_program(vert_shader_ID, frag_shader_ID);

    printf ("Shader ID is %d\n", program_ID);
    // Clean up memory (the first two variables you created)
    delete[] ( cstr_vertex_shader_source );
    delete[] ( cstr_fragment_shader_source );
}
